package keqing.gttopaddition.blur;
...
public class SimpleBlurHandler {
    ...
    private void refreshConfigCache() {
        configEnabled = GTTAConfig.blur.enabled;
        blurChat = GTTAConfig.blur.blurChat;
        dynamicBlur = GTTAConfig.blur.dynamicBlur;
        customShader = GTTAConfig.blur.customShader;
        debugMode = GTTAConfig.blur.debugMode;
        disableOnWorldLoad = GTTAConfig.blur.disableOnWorldLoad;
        disableOnDebugScreen = GTTAConfig.blur.disableOnDebugScreen;
        resetOnGuiSwitch = GTTAConfig.blur.resetOnGuiSwitch;

        // 更新列表缓存
        blacklistCache.clear();
        blacklistCache.addAll(Arrays.asList(GTTAConfig.blur.blacklist));

        whitelistCache.clear();
        whitelistCache.addAll(Arrays.asList(GTTAConfig.blur.whitelist));
    }
    ...
    private void startBlur() {
        ...
        try {
            if (!isBlurActive) {
                ...
                if (debugMode) {
                    GTTOPAddition.LOGGER.info("Blur effect started");
                }
            }
        } catch (Exception e) {
            if (debugMode) {
                GTTOPAddition.LOGGER.error("Failed to load blur shader: " + e.getMessage());
            }
        }
    }
    ...
    private void stopBlur() {
        if (isBlurActive) {
            ...
            if (debugMode) {
                GTTOPAddition.LOGGER.info("Blur effect stopped");
            }
        }
    }
    ...
    public void onGuiOpen(GuiOpenEvent event) {
        ...
        if (debugMode && event.getGui() != null) {
            GTTOPAddition.LOGGER.info("GUI opened: " + event.getGui().getClass().getName() +
