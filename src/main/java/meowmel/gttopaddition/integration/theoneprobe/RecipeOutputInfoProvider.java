...
import meowmel.gttopaddition.GTTAConfig;
import meowmel.gttopaddition.api.gui.impl.FluidStyle;
import meowmel.gttopaddition.api.utils.ItemMeta;
import meowmel.gttopaddition.api.utils.TranslationUtils;
...
public class RecipeOutputInfoProvider extends CapabilityInfoProvider<IWorkable> {
    ...
    protected void addProbeInfo(IWorkable capability, IProbeInfo probeInfo, EntityPlayer player, TileEntity tileEntity, IProbeHitData iProbeHitDat) {
        ...
        boolean showDetailed = fluids.size() + items.size() <= GTTAConfig.itemFluidDetailLimit && !player.isSneaking();
        IProbeInfo mainPanel = probeInfo.vertical()
                .text(TranslationUtils.topTranslate("gtqt.top.outputs"))
    ...
    private <T> void addOutputs(List<T> list, IProbeInfo panel, Function<T, IElement> getElement) {
        ...
        for (var entry : list) {
            if (idx >= GTTAConfig.maxEntriesToShowInRow) break;
    ...
    private IProbeInfo createHorizontalLayout(IProbeInfo mainPanel) {
        return mainPanel.horizontal(new LayoutStyle().spacing(GTTAConfig.rowDistanceSeperation));
    }
